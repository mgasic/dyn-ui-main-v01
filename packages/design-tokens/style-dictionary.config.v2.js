...
StyleDictionary.registerFormat({
  ...
  formatter: ({ dictionary }) => {
    ...
    const lightTokens = tokens.filter(t => !t.path.includes('darkTheme') && !t.path.includes('dark'));
    const darkTokens = tokens.filter(t => t.path.includes('darkTheme') || t.path.includes('dark'));
    ...
    if (darkTokens.length > 0) {
      ...
      darkTokens.forEach(token => {
        // Remove 'darkTheme' or 'dark' from path to match light token names
